...
#     apps_api = AppsApi(magicmock)
#     apps_api.driver = asyncmock
#     return apps_api
...
#         apps = AppsApi(magicmock, ocs_version="1")
#         assert apps.client == magicmock

#     async def test_get_app(self, apps: AppsApi, random: str):
#         await apps.get(random)
#         expected = [call.get(path=f"/cloud/apps/{random}", data=None, headers=None)]
#         apps.driver.assert_has_calls(expected)

#     async def test_apps_list(self, apps: AppsApi):
#         await apps.get_all()
#         expected = [
...
#     async def test_list_filter(self, apps: AppsApi):
#         await apps.get_all(filter="FILTER")
#         expected = [
...
#     async def test_list_enabled(self, apps: AppsApi):
...
#     async def test_list_disabled_pre_31(self, apps: AppsApi):
#         apps.driver.get.return_value = {"apps": {1: "app_1", 2: "app_2"}}
...
#     async def test_list_disabled_31_and_later(self, apps: AppsApi):
#         apps.driver.get.return_value = {"apps": ["app_1", "app_2"]}
#         response = await apps.list_disabled()
#         assert isinstance(response, list)
#         assert ["app_1", "app_2"] == response
...
#     async def test_enable_app(self, apps: AppsApi, random: str):
...
#     async def test_enable_password_confirm(self, apps: AppsApi, random: str):
#         apps.driver.post.side_effect = NextcloudForbiddenError(
...
#     async def test_disable_password_confirm(self, apps: AppsApi, random: str):
#         apps.driver.delete.side_effect = NextcloudForbiddenError(
#             "Password confirmation is required."
#         )
#         with pytest.raises(NextcloudForbiddenError):
...
#     async def test_disable_app(self, apps: AppsApi):
#         _app = sha1().hexdigest()
