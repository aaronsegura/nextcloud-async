...
1) Create a new branch based on `master` branch.
2) Fetch all dev dependencies.
    * Install required python modules using : **uv sync**
3) Ensure tests pass by running them using [`pytest`](https://doc.pytest.org/en/latest/index.html).
4) Add your changes.
5) Follow [Ruff](https://docs.astral.sh/ruff) code formatting, as configured in `pyproject.toml`
